...
こんにちは。shinfkdです。@<br>{}
普段はとある目黒のWeb企業で、人事としてエンジニア採用担当をやっています。@<br>{}
採用担当であるがゆえに、普段はほとんどコードを書く仕事をしておりません。@<br>{}
が、新卒採用イベントなどにはよく行きます。@<br>{}
人事が新卒採用イベントに何を持っていくのか…?@<br>{}
そう、iPad Proです。@<br>{}
...
すみません、、、嘘をつきました。@<br>{}
プレゼン資料を見せたり、オフィスの様子を見せるのに、iPad Proはとても便利なのですが、さすがにメモを取るのには苦労をしてしまいます。@<br>{}
PCでメモをとるのとは違い、タッチパネルで文字入力をするのには、どうしても時間がかかってしまうのです。@<br>{}
でも、待ってください。iPad Proにはアレがあります。@<br>{}
そう、Apple Pencilです。



そう思った私は、思い立ったが吉日とばかりに、Apple Pencilをポチりました。@<br>{}
...
Apple Pencilは2015年の11月に発売され、2017年9月現在のところ、対応しているのはiPad Proのみとなっています。@<br>{}
...
上述もしたとおり、Apple Pencilと指とでは、その取得できる情報に違いがあります。@<br>{}
具体的には、Apple Pencilを利用した場合、@<tt>{Altitude(高度)}と@<tt>{Azimuth(方位角)}、それから@<tt>{Force(筆圧)}の3つが取得できるようになります。



//image[azimuth_altitude][Altitude&Azimuth][scale=0.32]{
//}




Altitudeは、高さを意味します。@<br>{}
図3.1を見ていただくとわかるように、Cをペン先と見た場合に、Altitudeはペンの傾きを意味しています。@<br>{}
またAzimuthは、方位角を意味します。@<br>{}
...
上述した情報を取得するには、指で画面をタッチした場合と同じく、UITouchクラスを用います。@<br>{}
...
UITouchTypeに、iOS9.1から新たな種別であるStylusが追加されました。@<br>{}
これにより、Apple Pencilの場合のみを識別して値を取得することができます。


//list[UITouchType][UITouchType]{
@available(iOS 9.0, *)
...
//}
 * @<strong>{direct}: 指でタッチした場合
 * @<strong>{indirect}: 画面以外でデバイスを用いてタッチした場合
 * @<strong>{stylus}: Apple Pencilを用いた場合
...
Altitudeは、@<tt>{UITouch.altitudeAngle}より取得することができます。
Altitudeの値はラジアンで表現され、ペンがiPadのスクリーン面に対して水平になったときに値は0となり、スクリーンに対して直角のときに π/2となります。



//image[altitude_angle][AltitudeAngle]{
//}



//list[UITouch.altitudeAngleの取得][UITouch.altitudeAngleの取得]{
...
AzimuthをAltitudeと同じくラジアンで取得するには、@<tt>{UITouch.azimuthAngleInView}を使用します。@<br>{}
方位角は、画面右を0とし、そこから時計回りにペンを向けた場合値が増えていき、6時の方向でπ/2、9時でπとなります。反時計回りの場合は、マイナス値となり、0時で -π/2、9時でπとなります。



//image[azimuth_angle][AzimuthAngle]{
//}



//list[UITouch.azimuthAngleInViewの取得][UITouch.azimuthAngleInViewの取得]{
override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
...
Azimuthをベクトル値で取得するには、@<tt>{UITouch.azimuthUnitVector}を使用します。



//image[azimuth_vector][AzimuthVector]{
//}



//list[UITouch.azimuthUnitVectorの取得][UITouch.azimuthUnitVectorの取得]{
override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
...
筆圧は、Apple Pencilをスクリーンに押し付ける強さによって上下し、取得するには @<tt>{UITouch.force} を使用します。@<br>{}
この値は、iPad Proの場合、Apple Pencilを使わずに指で押した場合は常に0になり取得できませんが、3D Touchを搭載しているiPhoneの場合は取得可能になります。


//list[　UITouch.forceの取得][　UITouch.forceの取得]{
override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
...
さて、ここまでApple Pencilの特性について解説をしてまいりましたが、実際に画面に線を引いてみましょう。@<br>{}
まずは、Apple Pencilの特性を使わず、指でも書けるものから。@<br>{}
...
Apple Pencilの特徴で書いた通り、Apple Pencilは、指で書いたのに比べて倍の回数である秒間240回のスキャンを行います。@<br>{}
これは通常アプリにタッチが届けられるよりも高いレートであり、多くのアプリはこれほどまでの正確性を必要としておらず、余計なオーバーヘッドを招きたくありません。@<br>{}
そのため、通常の @<tt>{touches}で得られるタッチはApple Pencilがスキャンした全ての情報を含んでいません。



しかし、Apple Pencilが活躍するシーンである、ペインティングアプリケーションなどではより精緻なスキャン情報を必要とします。@<br>{}
そこで使われるのが @<tt>{UIEvent.coalescedTouches} メソッドです。@<br>{}
このメソッドを用いると、システムが前回受信したものの、配信されていない追加のタッチ情報を得ることができます。
実際に利用して書き換えたコードが下記になります。


//list[coalescedTouches][coalescedTouches]{
override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
    guard let touch = touches.first else { return }
...
次に、ペンの方位角と線を引く方向により、線の太さを変えるための記述です。@<br>{}
文章だけですとわかりづらいかと思いますので、下記の図をご覧ください。



//image[azimuth_line_width][AzimuthLineWidth]{
//}
...
//list[AzimuthlineWidth][AzimuthlineWidth]{
private func lineWidth(_ touch: UITouch) -> CGFloat {
    // ペンの傾きにより線の太さを決める
...
最後に、筆圧です。筆圧についてはすごく単純で、最後に筆圧自身を係数として掛けています。@<br>{}
ただし注意するのが、iPad Proの場合に @<tt>{UITouch.force} は指を使用すると常に0になるという点です。また、Apple Pencilを用いた場合でも、値は0から始まるため、普通にそのまま掛け合わせてしまうと非常に線が細くなってしまいます。そのため、両方の課題を解決するため、1以上の場合のみ実行するよう分岐を入れます。
