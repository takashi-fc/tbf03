...
//}
 ...
 シンプルな動画再生機能は以上で実装できます。サンプルコードは以下の通りです。@<br>{}<br><br>
...
//}
 ...
 @<tt>{nextPageToken}があるときは、まだレスポンスが残っているので再度リクエストする必要があるのです。次のレスポンスは、@<tt>{nextPageToken}をパラメーターに含めてリクエストします。前のレスポンスを取得するときは、@<tt>{prevPageToken}をパラメーターに含める必要があります。今回は、テーブルビューで5件表示し、残りはボタン押下時に再取得するようにしてみましたので、後ほど載せるサンプルコードで確認してみてください。@<br>{}
